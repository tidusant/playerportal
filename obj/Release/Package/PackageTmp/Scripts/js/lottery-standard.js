...
$(function () {
	...
	$l.on("orderPlacedCalled", function (ev, wasSuccessful, orderID) {
		if (wasSuccessful) {
			...
			$.jStorage.set("lottoToken", lottery.token);
			$(".welcome-text").text("Welcome " + wallet.player.username + "! ($" + wallet.player.balance + ")");
	...
	$w.on("getLotteryTokenCalled", function (ev, resp) {
		if (resp.wasSuccessful) {
			...
			$.jStorage.set("lottoToken", resp.token);
			checkUI();
	...
	$("#addbet-action").on("click", function () {
		...
		if (selectedDraws.length <= 0) {
			...
			for (var i = 0; i < selectedDraws.length; i++) { // did start at 1
				...
				//if(sbet.replace("$", "") > 0 && bbet.replace("$", "") > 0)
				//	gameTypeIDs.push(hAbbr);
			}
			...
			lottery.placeBets(bets, gameTypeIDs,1, sbet.replace("$", ""), bbet.replace("$", ""));
...
function updateBalance() {
	var newBalance = parseFloat(wallet.player.balance) - parseFloat(betTotal());
	wallet.player.balance = newBalance;
	$(".balance-text").text(currencySymbol + wallet.player.balance.toFixed(numDecimals));
}
...
if (typeof setupMoreLessChildren == 'function') {
		setupMoreLessChildren("#drawing-list", "div.drawing-item");
	}
	globalGames = games;
