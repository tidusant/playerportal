...
var walletInit = (function (baseUrl, _$, key, currencyCulture, languageCulture) {
    ...
    var EVENT_TRANSACTIONS_SEARCHED = "transactionsSearched";
    ...
    this.getPlayer = function (username, password) {
		...
		console.log("ready for posting data");
                ...
                if (resp.wasSuccessful == false) {
                    ...
                    $.post(url, data, function (resp) {
                        if (resp == null) {
							...
							console.log("api respone successful");
                            serverTimeLocal = resp.serverTimeLocal;
                            serverTimeUTC = resp.serverTimeUTC;
                            if (resp.wasSuccessful == false) {
                                wallet = null;
                                ...
                                $.jStorage.flush();
                                $.jStorage.set("faillogin",faillogin+1);
                                if(faillogin>=2){
                                    //show captcha
                                    $('#captchaContent').show();
                                }
                                ...
                                $.jStorage.set("CafeID", resp.CafeID);
                                ...
                                // window.location.href = siteurl + 'games/lotto';
                                window.location.href = siteurl + 'games/Casino';
                            }
                        }
                    });
                }
...
console.log("all done");
    ...
    this.getPlayerDetails = function () {
        ...
        var $m = $(that);
        var url = siteurl;
        url = url + "Account/GetPlayerDetails";
        var data = {
                    ...
                    $.post(url, data, function (resp) {
                        if (resp == null) {
                            ...
                            if (resp.wasSuccessful == false) {
                                ...
                                loadUser();
                                //$.jStorage.get("refundUrl").trim() == undefined ? '' : $("refundUrl").trim() && $.jStorage.get("returnFromLobby") == true
                                //if ($.jStorage.get("refundUrl").trim() != "" && $.jStorage.get("returnFromLobby") == true) {
                                if ($.jStorage.get("refundUrl") != "" && $.jStorage.get("returnFromLobby") == true) {

                                    var refundUrl = $.jStorage.get("refundUrl");
                                    $.jStorage.set("refundUrl", "");
                                    $.jStorage.set("returnFromLobby", false);
                                    parent.location.href = refundUrl;
                                }
                    ...
                    $.post(url, data, function (resp) {
                        if (resp == null) {
                            ...
                            if (resp.wasSuccessful == false) {
                                ...
                                $.jStorage.set("OTPIsUsed", resp.IsUsed);
    ...
    this.editDetails = function (firstname, lastname, gender, birthday, emailaddress, cellphone, phonenumber, employer, address1, address2, address3, address4, locality, region, postcode, country) {
        ...
        var data = {
            Key: fkey,
            PlayerID: PlayerID, Name: firstname, lastname: lastname, Gender: gender, EmailID: emailaddress, Phone: cellphone, AlternatePhone: phonenumber, Employer: employer, Address: address1, AddressLine1: address2, AddressLine2: address3, AddressLine3: address4, City: locality, State: region, ZipCode: postcode, Country: country
        };
        $.post(url, data, function (resp) {
            if (resp == null) {
                wallet = null;
    ...
    this.transactions = function (CardNumber, sDate, eDate) {
        ...
        var $m = $(that);
        var url = siteurl;
        url = url + "Account/GetPlayerTransaction";
        var data = {
            StartDate: sDate,
            EndDate: eDate,
            CardNumber: CardNumber
                ...
                if (resp.wasSuccessful == false) {
                    ...
                    $.post(url, data, function (resp) {
                        if (resp == null) {
                            ...
                            // if (resp.token != null && resp.token != "") wallet.token = resp.token;
                            if (resp.wasSuccessful == false) {
                                $m.trigger(EVENT_TRANSACTIONS_SEARCHED, [resp]);
                                layer.close(pageii);
                                return false;
                                $m.trigger(EVENT_ERROR_OCCURRED, "Transaction not found!", true);  
