...
$(function () {
    ...
    $w.on("errorOccurred", function (ev, error, isFatal) {
        if(wallet)$.jStorage.set("token", wallet.token);
    ...
    $w.off("editDetailsCalled").on("editDetailsCalled", function (ev, resp) {
    ...
    $w.off("transactionsSearched").on("transactionsSearched", function (ev, resp) {           
        $.jStorage.set("token", wallet.token);
        var transactions = resp.transactions;

        $(".transaction-history-body").empty();
        //create header
        var template = document.querySelector('#transaction-header');
        var newLine = document.importNode(template.content, true);
        $(".transaction-history-body").append(newLine);

        if (transactions.length > 0) {
            for (var i = 0; i < transactions.length; i++) {
                template = document.querySelector('#transaction-history-row-template');
                var newLine = document.importNode(template.content, true);
                var transaction = transactions[i];

                newLine.querySelector(".transaction-date").innerHTML = (transaction.createdOn);
    ...
    $w.on("transactionDetailsCalled", function (ev, resp) {
        ...
        if (details != null) {
            ...
            for (var i = 0; i < details.length; i++) {
                ...
                newLine.querySelector(".transaction-detail-order-houseName").innerHTML = order.house;
...
function getDetails() {
    //wallet.details(); 
    wallet.getPlayerDetails(); // added on 10-jan-2019 for player details load.
}
